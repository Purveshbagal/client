      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        ...
        <div className="mt-8 md:mt-12">
          <div className="mb-6 md:mb-8">
            ...
            <p className="text-gray-600 mt-2 text-sm md:text-base">Order from your favorite restaurants and get fresh meals delivered</p>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              {/* Pagination */}
              <div className="mt-8 md:mt-12 flex flex-col sm:flex-row justify-center items-center gap-3 md:gap-4">
                <button
                  onClick={() => setPage(Math.max(1, page - 1))}
                  disabled={page === 1}
                  className="w-full sm:w-auto px-4 md:px-6 py-2 bg-white border-2 border-gray-300 rounded-lg font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition text-sm md:text-base"
                >
                  ← Previous
                </button>
                <div className="flex items-center gap-2">
                  <span className="text-gray-700 font-semibold text-sm md:text-base">Page</span>
                  <div className="bg-white border-2 border-orange-500 rounded-lg px-3 md:px-4 py-2 font-bold text-orange-500 text-sm md:text-base">
                    {page}
                  </div>
                  <span className="text-gray-700 font-semibold text-sm md:text-base">of {totalPages}</span>
                </div>
                <button
                  onClick={() => setPage(Math.min(totalPages, page + 1))}
                  disabled={page >= totalPages}
                  className="w-full sm:w-auto px-4 md:px-6 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg font-semibold hover:from-orange-600 hover:to-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition text-sm md:text-base"
                >
                  Next →
                </button>
              </div>
            ...
            <div className="text-center py-8 md:py-12">
              ...
              <p className="text-gray-600 text-base md:text-lg">No restaurants found</p>
              <p className="text-gray-500 mt-2 text-sm md:text-base">Try adjusting your search criteria</p>
            </div>
          )}
        </div>
      </div>
